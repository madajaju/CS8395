...
[plantuml, rest-core-concepts-resource-vs-rpc, svg]
----
@startuml
title Resource-Oriented vs RPC-Style Endpoints

rectangle "RPC-style API" as RPC {
  rectangle "/getUser" as getUser
  rectangle "/createUser" as createUser
  rectangle "/doUserStuff" as doUserStuff
}

rectangle "Resource-oriented API" as RES {
  rectangle "/users" as users
  rectangle "/users/{id}" as userById
  rectangle "/users/{id}/orders" as userOrders
}

getUser -down[hidden]-> createUser
createUser -down[hidden]-> doUserStuff

users -down[hidden]-> userById
userById -down[hidden]-> userOrders

note bottom of RPC
  Verbs baked into paths,
  hard to generalize or guess.
end note

note bottom of RES
  Nouns and relationships:
...
[plantuml, rest-resource-modeling-urls, svg]
----
@startuml
...
[plantuml, rest-to-mcp-mapping, svg]
----
@startuml
title Mapping a REST Endpoint to an MCP Tool

left to right direction
rectangle "REST API" as REST {
  rectangle "POST /orders" as PostOrders
  rectangle "Request JSON:\n{ userId, items[] }" as RestReq
  rectangle "Response JSON:\n{ orderId, status, total }" as RestRes
  rectangle "Errors:\nINVALID_INPUT,\nINSUFFICIENT_STOCK" as RestErr
}

rectangle "MCP Tool" as MCP {
  rectangle "Tool: createOrder" as ToolName
  rectangle "Params:\nuserId: string\nitems: array" as ToolParams
  rectangle "Result:\norderId: string\nstatus: string\ntotal: number" as ToolResult
  rectangle "Errors:\nINVALID_INPUT,\nINSUFFICIENT_STOCK" as ToolErrors
}

PostOrders -down-> RestReq
PostOrders -down-> RestRes
PostOrders -down-> RestErr

ToolName -down-> ToolParams
ToolName -down-> ToolResult
ToolName -down-> ToolErrors

REST -right-> MCP : "Shared contract\n(semantics & schema)"
@enduml
----
